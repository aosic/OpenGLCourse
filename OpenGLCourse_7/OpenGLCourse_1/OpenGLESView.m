...
- (void)szh_setupTextureData {
    ...
    NSString *path2 = [[NSBundle mainBundle] pathForResource:@"flower" ofType:@"jpg"];
    ...
    //加载纹理
    if (read_jpeg_file(path1.UTF8String, &data1, &size1, &width1, &height1) < 0) {
        printf("%s\n", "decode fail");
    }
    
    if (read_jpeg_file(path2.UTF8String, &data2, &size2, &width2, &height2) < 0) {
        printf("%s\n", "decode fail");
    }
    
    //创建纹理
    _texture1 = createTexture2D(GL_RGB, width1, height1, data1);
    if (data1) {
        free(data1);
        data1 = NULL;
    }
    
    _texture2 = createTexture2D(GL_RGB, width2, height2, data2);
    if (data2) {
        free(data2);
        data2 = NULL;
    }
